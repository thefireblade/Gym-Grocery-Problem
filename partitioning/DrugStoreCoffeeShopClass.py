...
class DrugStoreCoffeeShops():
    ...
    def G_to_disjoint(self):
        if(self.G):
            self.gObj = GymGroceryGraph(len(self.G.nodes), self.n)
            store_starting_index = self.n + len(self.C[0])
            self.gObj.initVertices()
            self.k_closest = []
            self.k_closest.append({})
            self.k_closest.append({})
            ...
            for person in range(self.n):
                ...
                for neighbor in self.G.neighbors(person):
                    if neighbor < store_starting_index:
                        gyms.append(neighbor)
                    else:
                        stores.append(neighbor)
                if len(gyms) == 1:
                    self.gObj.union(gyms[0], person)
                    self.gObj.addEdge(gyms[0], person)
                else:
                    person_maps.append({
                        'shoplist': gyms,
                        'person': person
                    })
                    self.k_closest[0][person] = gyms
                if len(stores) == 1:
                    self.gObj.union(stores[0], person)
                    self.gObj.addEdge(stores[0], person)
                else:
                    person_maps.append({
                        'shoplist': stores,
                        'person': person
                    })            
                    self.k_closest[1][person] = stores
            self.person_maps = person_maps
